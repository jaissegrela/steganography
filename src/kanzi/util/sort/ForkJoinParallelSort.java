...
public class ForkJoinParallelSort implements IntSorter, ByteSorter {
	...
	public ForkJoinParallelSort(ForkJoinPool pool) {
		if (pool == null)
			throw new NullPointerException(
					"Invalid null fork/join pool parameter");
	 ...
	 * 
	 */
		private static final long serialVersionUID = 5972058541551107783L;
		...
		protected void sortBytes() {
			...
			SortTask lowerHalfTask = new SortTask(this.iSrc, this.startIdx,
					half, this.iDst);
			SortTask upperHalfTask = new SortTask(this.iSrc, this.startIdx
					+ half, this.size - half, this.iDst);
			...
			while ((idx1 < end1) && (idx2 < end2)) {
				dest[idx++] = (source[idx1] < source[idx2]) ? source[idx1++]
						: source[idx2++];
			}
			...
			System.arraycopy(dest, this.startIdx, source, this.startIdx,
					this.size);
		}
